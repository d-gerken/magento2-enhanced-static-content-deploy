...
namespace Dgerken\EnhancedStaticContentDeploy\Model;

use Dgerken\EnhancedStaticContentDeploy\Cache\StaticFileCache;
use Dgerken\EnhancedStaticContentDeploy\View\Publisher;
...
class Deployer
{
    ...
    /** @var \Dgerken\EnhancedStaticContentDeploy\View\Asset\Repository */
    private $assetRepo;
     ...
     * @param bool $isDryRun
     * @param StaticFileCache $contentHashManager
     */
    public function __construct(
        ...
        $isDryRun = false,
        StaticFileCache $contentHashManager
    ) {
        ...
        $this->parentTheme = [];
        $this->fileHashManager = $contentHashManager;
    ...
    public function deploy(ObjectManagerFactory $omFactory, array $locales)
    {
        ...
        $this->libFiles = $this->filesUtil->getStaticLibraryFiles();
        list($areas, $this->appFiles) = $this->collectAppFiles($locales);
        foreach ($areas as $area => $themes) {
            $this->emulateApplicationArea($area);
            foreach ($locales as $locale) {
                $this->emulateApplicationLocale($locale, $area);
                foreach ($themes as $themePath) {
                    $this->deployTheme($area, $locale, $themePath);
                }
            }
        }
    ...
    protected function deployTheme($area, $locale, $themePath)
    {
        $this->output->writeln("=== {$area} -> {$themePath} -> {$locale} ===");
        $this->output->writeln("=== Please wait... ===");
        $this->count = 0;
        $this->errorCount = 0;
        ...
        $assetRepo = $this->objectManager->create(
            'Dgerken\EnhancedStaticContentDeploy\View\Asset\Repository',
            [
                'design' => $design,
            ]
        );
        ...
        $ancestors = $this->findAncestors($area . Theme::THEME_PATH_SEPARATOR . $themePath);

        foreach ($this->appFiles as $info) {
            $inAncestor = false;
            list($fileArea, $fileTheme, ,$module, $filePath, $fullPath) = $info;
            if(!$this->fileHashManager->hasChange($fullPath, $area . $locale . $themePath))
                continue;
            for ($i = 0; $i < count($ancestors); $i++)
            {
                if($ancestors[$i] == $fileArea . Theme::THEME_PATH_SEPARATOR . $fileTheme){
                    $inAncestor = true;
                    break;
                }
            }
            if(($fileArea == $area || $fileArea == 'base')
                && ($inAncestor || $fileTheme == '' || $fileTheme == $themePath))
            {
        ...
        foreach ($this->libFiles as $filePath) {
            if(!$this->fileHashManager->hasChange(BP . "/lib/web/" . $filePath, $area . $locale . $themePath))
                continue;
    ...
    private function emulateApplicationArea($areaCode)
    {
        ...
        $this->assetRepo = $this->objectManager->get('Dgerken\EnhancedStaticContentDeploy\View\Asset\Repository');
    ...
    private function deployFile($filePath, $area, $themePath, $locale, $module)
    {
        ...
        foreach ($this->alternativeSources as $name => $alternative) {
            $hasAlternativeExtensionName = false;
            $alternativeExtensionNames = $alternative->getAlternativesExtensionsNames();
            foreach($alternativeExtensionNames as $alternativeName)
            {
                if($alternativeName === $extension){
                    $hasAlternativeExtensionName = true;
                    break;
                }
            }

            if ($hasAlternativeExtensionName && strpos(basename($filePath), '_') !== 0) {
    ...
    /**
     * @return \Magento\Framework\View\Asset\PreProcessor\AlternativeSourceInterface[]
     */
